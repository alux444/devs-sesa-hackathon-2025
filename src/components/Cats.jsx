  ...
  return (
    <div>
      <div>
        ...
        {!catImageUrl && <img src={loadingSpinner} alt="loading" />}
      </div>

      <div>
        <p>Cat image with 'orange' and 'cute' tags</p>
        {otherCatImageUrl && <img className="smallImage" src={otherCatImageUrl} alt="cat" />}
        {!otherCatImageUrl && <img src={loadingSpinner} alt="loading" />}
      </div>

      <div className="maxWidthScreen">
        <button onClick={() => console.log(possibleCatTags)}>
          Print possible cat tags in developer console!
        </button>
      </div>
    </div>
  );
